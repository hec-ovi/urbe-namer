...
Status: v0.4.8. NPC type shape is stable, simulation consumes it. The world-state view is verified against atlas blueprint v0.14 and accepts the additive v0.15 hydrology document; everything naming does not name passes through untouched.
...
- `COVERAGE_ERROR`: repair loop exhausted; naming output is incomplete, carries invalid required metadata, is duplicated or outside the sign alphabet, or typing output stays ungrounded (references the world lacks, name pool below minimum).
