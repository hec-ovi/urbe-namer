import type { Business, WorldState } from "../types.js";
...
export function exportBusinesses(world: WorldState): Business[] {
  schemas.assert("world-state.schema.json", world, "INVALID_WORLD", "world state");
  if (!world.meta.naming) {
    throw new NamingError("INVALID_WORLD", "businesses export needs a named world (run the naming pass first)");
  }
