...
- **A named world** ([schema/named-world.schema.json](schema/named-world.schema.json)): the input document untouched except that every selected nameable gains a `name`, with a required `meta.naming` record of theme, model and canonical UTC timestamp. It is saved alongside the placeholder version, never over it. Names are unique case-insensitively inside their namespace, themed chains stay possible through branch-qualified names, and every name spells in the sign alphabet (letters, digits, space and a few marks, 32 characters at most) so signs and screens letter it verbatim.
...
It names the world that [atlas](https://github.com/hec-ovi/urbe-atlas) generates and grounds its NPC types in [simulation](https://github.com/hec-ovi/urbe-population) demographics. The named world and the type set go to [simulation](https://github.com/hec-ovi/urbe-population), which instantiates people from them, [quests](https://github.com/hec-ovi/urbe-quests), which writes the story on top, and [engine](https://github.com/hec-ovi/urbe-engine), which shows the names in the world; the businesses list goes to [materials](https://github.com/hec-ovi/pbrforge), which brands its ad screens with them. The full picture lives in [urbe](https://github.com/hec-ovi/urbe).
